...
use crate::common::{initials, read_from_ls, user_color_class, AppHeader, Avatar};
...
pub fn PaymentPage(project_id: Uuid, expense_id: i32) -> Element {
    ...
    let ls = read_from_ls();
    let stored_user_id =
        ls.projects.iter().find(|p| p.project_id == project_id).and_then(|p| p.user_id);

    let mut expense = use_resource(move || async move { get_expense_by_id(expense_id).await });
    let mut payments =
        use_resource(move || async move { get_payments_by_expense_id(expense_id).await });
    ...
    rsx! {
        div { class: "container overflow-auto p-4 max-w-md w-full mx-auto flex flex-col gap-4",
            ...
            match (&*expense.read(), &*payments.read(), &*users.read()) {
                ...
                AppHeader {
                    title: "{exp_name}",
                    back_button_route: Route::ProjectDetails {
                        project_id,
                    },
                    sub_title: "{exp_date.format(\"%d/%m/%Y\")}",
                    details { class: "dropdown dropdown-end",
                        ...
                        ul { class: "menu dropdown-content bg-base-100 rounded-box w-40 shadow z-10 p-2",
                            li {
                                a { onclick: move |_| show_edit.set(true), "Modifier" }
                            }
                            li {
                                a { class: "opacity-50", "Supprimer" }
                            }
                        }
                    }
                }
